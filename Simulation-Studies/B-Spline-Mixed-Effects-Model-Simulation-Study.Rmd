---
title: "B-Spline Mixed Effects Model Simulation Study"
...
```{r}
generate.data <- function(n.participants, n.measurements, fixed.coefficient.1, fixed.coefficient.2, fixed.coefficient.3, sd.random.intercept, sd.error) {
  ...
  fixed.effect <- c(x.matrix %*% c(fixed.coefficient.1, fixed.coefficient.2, fixed.coefficient.3))
...
estimate.model.and.get.coefficients.nlme <- function(data) {
  ptm <- proc.time()
  model <- lme(
    fixed = y ~ 0 + bbase(x, xl = 0, xr = 1, nseg = 1, bdeg = 2),
    data = data,
...
estimate.model.and.get.coefficients.lme4 <- function(data) {
  ptm <- proc.time()
  model <- lmer(
    formula = y ~ 0 + bbase(x, xl = 0, xr = 1, nseg = 1, bdeg = 2) + (1 | participant.id),
    data = data,
    REML = FALSE
  )
  time <- proc.time() - ptm
...
estimate.model.and.get.coefficients.gamlss1 <- function(data) {
  ...
  ptm <- proc.time()
  model <- gamlss(
    formula = y ~ 0 + re(fixed = ~ 0 + bbase(x, xl = 0, xr = 1, nseg = 1, bdeg = 2), random = ~ 1 | participant.id),
    sigma.formula = ~ 1,
    family = NO(
      mu.link = "identity",
      sigma.link = "log"
...
estimate.model.and.get.coefficients.gamlss2 <- function(data) {
  ...
  ptm <- proc.time()
  model <- gamlss(
    formula = y ~ 0 + bbase(x, xl = 0, xr = 1, nseg = 1, bdeg = 2) + random(participant.id),
    sigma.formula = ~ 1,
    family = NO(
      mu.link = "identity",
      sigma.link = "log"
...
estimate.model.and.get.coefficients.mgcv1 <- function(data) {
  ptm <- proc.time()
  model <- gam(
    formula = y ~ 0 + bbase(x, xl = 0, xr = 1, nseg = 1, bdeg = 2) + s(participant.id, bs = "re"),
    data = data,
    method = "ML"
  )
  time <- proc.time() - ptm
...
estimate.model.and.get.coefficients.mgcv2 <- function(data) {
  ptm <- proc.time()
  model <- gamm(
    formula = y ~ 0 + bbase(x, xl = 0, xr = 1, nseg = 1, bdeg = 2),
...
estimate.model.and.get.coefficients.optim <- function(data) {
  ...
  fun.split <- function(split.data, par) {
    ...
    y <- split.data$y
    x.matrix <- bbase(x, xl = 0, xr = 1, nseg = 1, bdeg = 2)
    fixed.effect <- c(x.matrix %*% par[seq(3)])
    log.sd.random.intercept <- par[[4]]
    log.sd.error <- par[[5]]
  ...
  ptm <- proc.time()
  model <- optim(
    par = c(fixed.coefficient.1, fixed.coefficient.2, fixed.coefficient.3, log(sd.random.intercept), log(sd.error)),
    fn = fun.optim,
    method = "BFGS"
  )
  time <- proc.time() - ptm
  result <- c(
    ...
    sd.random.intercept = exp(model$par[[4]]),
    sd.error = exp(model$par[[5]]),
    time = time[[3]]
  )
  return(result)
}
...
estimate.model.and.get.coefficients.gls <- function(data) {
  ...
  fun.gls <- function(ratio) {
    ...
    z <- backsolve(chol.matrix, y)
    log.sqrt.det <- sum(log(diag(chol.matrix)))
    loglikelihood <- rowSums(-0.5 * z %*% z) / var.residuals - log.sqrt.det - log(2 * pi) * (n.participants * n.measurements / 2)
...
parameters <- tibble(
  ...
  fixed.coefficient.1 = c(1, 1, 1, -0.5, 1, 1),
  fixed.coefficient.2 = c(-1, -1, -1, 0.5, -1, -1),
  fixed.coefficient.3 = c(2, 2, 2, 1.5, 2, 2),
...
ggplot(
  ...
  data = estimated.parameters %>%
    select(-time) %>%
    pivot_longer(c("fixed.coefficient.1", "fixed.coefficient.2", "fixed.coefficient.3", "sd.random.intercept", "sd.error")) %>%
...
estimated.parameters <- parameters %>%
  ...
  reframe(
    generate.data(
      ...
      estimate.model.and.get.coefficients.lme4() %>%
      as_tibble_row(),
    .by = c(parameter.set, run)
  )
estimated.parameters
...
ggplot(
  ...
  data = estimated.parameters %>%
    ...
    reframe(
      expand_grid(
        parameter.1 = c("fixed.coefficient.1", "fixed.coefficient.2", "fixed.coefficient.3", "sd.random.intercept", "sd.error"),
        parameter.2 = c("fixed.coefficient.1", "fixed.coefficient.2", "fixed.coefficient.3", "sd.random.intercept", "sd.error")
      ),
      correlation = c(cor(data.frame(fixed.coefficient.1, fixed.coefficient.2, fixed.coefficient.3, sd.random.intercept, sd.error)))
    )
) +
  geom_tile(alpha = 0.5) +
  geom_text() +
